...
(function(){
  ...
  register('renderTransferLog', function(){ensureTransferLogPrefs();DB.transferler=DB.transferler||[];const list=el('transfer-log-liste'),sec=el('transfer-log-msec');if(!list)return;if(sec)sec.style.display=DB.transferler.length?'':'none';renderStatus();if(!DB.transferler.length){list.innerHTML='<div style="color:var(--text3);font-size:12px;padding:8px">Henüz transfer yok</div>';return;}const map={};(DB.hesaplar||[]).forEach(function(h){map[h.id]=h.ad;});const selected=typeof tblFiltreOkuMulti==='function'?tblFiltreOkuMulti('transferLog','filtre'):[];_transferLogFiltreLabelGuncelle(map,selected);let rows=DB.transferler.slice().reverse();if(selected.length)rows=rows.filter(function(t){return selected.some(function(f){if(f.indexOf('h:')===0){const id=f.slice(2);return t.kaynakId===id||t.hedefId===id;}const pb=f.slice(2);return(t.kTip==='nakit'&&t.kaynakPb===pb)||(t.hTip==='nakit'&&t.hedefPb===pb);});});const filter=st();if(filter)rows=rows.filter(function(t){return possible(t)===(filter==='ok');});if(!rows.length){list.innerHTML='<div style="color:var(--text3);font-size:12px;padding:8px 4px">Kayıt yok</div>';return;}list.innerHTML=rows.map(function(t){const ok=possible(t),pb=t.kaynakPb||t.hedefPb||'TRY';return '<div class="rf-transfer-row"><span class="state '+(ok?'ok':'no')+'" title="'+(ok?'Şu an tekrar yapılabilir':'Şu an tekrar yapılamaz')+'"></span><div class="main"><div class="route">'+esc(side(t,'k'))+' <span style="color:var(--text3)">→</span> '+esc(side(t,'h'))+'</div>'+(t.aciklama?'<div class="note">'+esc(t.aciklama)+'</div>':'')+'</div><div class="amount"><div class="mono" style="font-weight:800;color:var(--teal)">'+fmtCur(t.tutar,pb)+'</div><div style="color:var(--text3);font-size:10px">'+fmtDate(t.tarih)+'</div></div><div class="act"><button class="ib rf-transfer-tekrar-btn" data-id="'+t.id+'" title="Bu transferi tekrarla">🔁</button><button class="ib rf-transfer-sil-btn" data-id="'+t.id+'" title="Sil">✕</button></div></div>';}).join('');list.querySelectorAll('.rf-transfer-tekrar-btn').forEach(function(btn){btn.addEventListener('click',function(){tekrarlaTransfer(btn.getAttribute('data-id'));});});list.querySelectorAll('.rf-transfer-sil-btn').forEach(function(btn){btn.addEventListener('click',function(){deleteTransfer(btn.getAttribute('data-id'));});});});
